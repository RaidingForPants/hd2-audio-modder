...
class MusicSegment(HircEntry):
    ...
    def from_memory_stream(cls, stream):
        ...
        stream.seek(stream.tell() - 1)
        entry.unused_sections.append(
            stream.read(5 * n + 1 + 12 + 4)
        )  # the 4 is the count of state props, state chunks, and RTPC, which are currently always 0
    ...
    def set_data(self, duration=None, entry_marker=None, exit_marker=None):
        ...
        if entry_marker is not None:
            self.entry_marker[1] = entry_marker
        if exit_marker is not None:
            self.exit_marker[1] = exit_marker
        self.modified = True
            ...
            [
                ...
                b"".join(
                    [
                        b"".join(
                            [
                                x[0].to_bytes(4, byteorder="little"),
                                struct.pack("<d", x[1]),
                                x[2],
                            ]
                        )
...
class TrackInfoStruct:
    ...
    def __init__(self):
        self.track_id = self.source_id = self.event_id = self.play_at = (
            self.begin_trim_offset
        ) = self.end_trim_offset = self.source_duration = 0
        self.play_at_old = self.begin_trim_offset_old = self.end_trim_offset_old = (
            self.source_duration_old
        ) = 0
        self.modified = False
    ...
    def set_data(
        self,
        play_at=None,
        begin_trim_offset=None,
        end_trim_offset=None,
        ...
        if begin_trim_offset is not None:
            self.begin_trim_offset = begin_trim_offset
        if end_trim_offset is not None:
            self.end_trim_offset = end_trim_offset
...
class WwiseBank(Subscriber):
    ...
    def generate(self, audio_sources, eventTrackInfo):
        ...
        bank_generation_progress_window = ProgressWindow(
            "Generating Soundbanks", max_progress
        )
            ...
            for source in entry.sources:
                ...
                if source.plugin_id == VORBIS:
                    ...
                    if source.stream_type == PREFETCH_STREAM:
                        ...
                        didx_array.append(
                            struct.pack(
                                "<III", source.source_id, offset, source.mem_size
                            )
                        )
                        ...
                        didx_array.append(
                            struct.pack("<III", source.source_id, offset, audio.size)
                        )
                        offset += audio.size
        if len(didx_array) > 0:
            data += "DIDX".encode("utf-8") + (12 * len(didx_array)).to_bytes(
                4, byteorder="little"
            )
            data += b"".join(didx_array)
            data += "DATA".encode("utf-8") + sum([len(x) for x in data_array]).to_bytes(
                4, byteorder="little"
            )
            data += b"".join(data_array)

        hierarchy_section = self.hierarchy.get_data()
        data += "HIRC".encode("utf-8") + len(hierarchy_section).to_bytes(
            4, byteorder="little"
        )
...
class FileReader:
    ...
    def to_file(self, path):
        ...
        stream_file = MemoryStream()
        self.num_files = (
            len(self.wwise_streams) + 2 * len(self.wwise_banks) + len(self.text_banks)
        )
        self.num_types = 0
    ...
    def load(self, toc_file, stream_file):
                ...
                if "DIDX" in bank.chunks.keys():
                    ...
                    for e in hirc.entries.values():
                        for source in e.sources:
                            if (
                                source.plugin_id == VORBIS
                                and source.stream_type == BANK
                                and source.source_id not in self.audio_sources
                            ):
                                audio = AudioSource()
                ...
                for chunk in bank.chunks.keys():
                    if chunk not in ["BKHD", "DATA", "DIDX", "HIRC"]:
                        entry.bank_misc_data = (
                            entry.bank_misc_data
                            + chunk.encode("utf-8")
                            + len(bank.chunks[chunk]).to_bytes(4, byteorder="little")
                            + bank.chunks[chunk]
                        )
        ...
        for bank in self.wwise_banks.values():
            if (
                bank.dep == None
            ):  # can be None because older versions didn't save the dep along with the bank
        ...
        if (
            len(self.wwise_banks) == 0 and len(self.wwise_streams) > 0
        ):  # 0 if patch was only for streams
        ...
        for bank in self.wwise_banks.values():
            for entry in bank.hierarchy.entries.values():
                for source in entry.sources:
                    if (
                        source.plugin_id == VORBIS
                        and source.stream_type in [STREAM, PREFETCH_STREAM]
                        and source.source_id not in self.audio_sources
                    ):
                        try:
                            stream_resource_id = murmur64_hash(
                                (
                                    os.path.dirname(bank.dep.data)
                                    + "/"
            ...
            for entry in bank.hierarchy.entries.values():
                for source in entry.sources:
                    try:
                        if (
                            source.plugin_id == VORBIS
                            and self.audio_sources[source.source_id]
                            not in bank.get_content()
                        ):  # may be missing streamed audio if the patch didn't change it
                    ...
                    try:
                        if info.source_id != 0:
                            self.audio_sources[info.source_id].set_track_info(
                                info, notify_subscribers=False, set_modified=False
                            )
                    except:
                        continue
    ...
    def load_deps(self):
        ...
        if GAME_FILE_LOCATION != "":
            archive_file = os.path.join(
                GAME_FILE_LOCATION, strip_patch_index(self.name)
            )
        if not os.path.exists(archive_file):
            warning = PopupWindow(
                message="This patch may have been created using an older version of the audio modding tool and is missing required data. Please select the original game file to load required data."
            )
            warning.show()
        ...
        for key, bank in self.wwise_banks.items():
            ...
            for hierarchy_entry in bank.hierarchy.entries.values():
                for source in hierarchy_entry.sources:
                    if source.plugin_id == VORBIS and source.stream_type in [
                        STREAM,
                        PREFETCH_STREAM,
                    ]:
                        stream_resource_id = murmur64_hash(
    ...
    def play_audio(self, sound_id, sound_data, callback=None):
        ...
        if not os.path.isfile(f"{filename}.wav"):
            with open(f"{filename}.wem", "wb") as f:
                f.write(sound_data)
            subprocess.run(
                [VGMSTREAM, "-o", f"{filename}.wav", f"{filename}.wem"],
        ...
        self.audio_process = self.audio.open(
    ...
    def downmix_to_stereo(self, data, channels, channel_width, frame_count):
        ...
        if channels == 4:
            for index, frame in enumerate(arr):
                stereo_array[index][0] = int(
                    0.42265 * frame[0] + 0.366025 * frame[2] + 0.211325 * frame[3]
                )
                stereo_array[index][1] = int(
                    0.42265 * frame[1] + 0.366025 * frame[3] + 0.211325 * frame[2]
                )
...
class FileHandler:
    ...
    def dump_as_wem(self, file_id):
        output_file = filedialog.asksaveasfile(
            mode="wb",
            title="Save As",
            initialfile=(str(file_id) + ".wem"),
            defaultextension=".wem",
            filetypes=[("Wwise Audio", "*.wem")],
        )
    ...
    def dump_multiple_as_wem(self, file_ids, with_seq: bool = False):
        folder = filedialog.askdirectory(title="Select folder to save files to")

        progress_window = ProgressWindow(
            title="Dumping Files", max_progress=len(file_ids)
        )
    ...
    def dump_all_as_wem(self):
        ...
        progress_window = ProgressWindow(
            title="Dumping Files", max_progress=len(self.file_reader.audio_sources)
        )
        progress_window.show()
        ...
        if os.path.exists(folder):
            for bank in self.file_reader.wwise_banks.values():
                subfolder = os.path.join(
                    folder, os.path.basename(bank.dep.data.replace("\x00", ""))
                )
        ...
        if os.path.exists(folder):
            for bank in self.file_reader.wwise_banks.values():
                ...
                for audio in bank.get_content():
                    save_path = os.path.join(subfolder, f"{audio.get_id()}")
                    progress_window.set_text(
                        "Dumping " + os.path.basename(save_path) + ".wav"
                    )
                    with open(save_path + ".wem", "wb") as f:
                        f.write(audio.get_data())
                    subprocess.run(
    ...
    def load_archive_file(self, initial_dir: str | None = None):
        archive_file = askopenfilename(title="Select archive", initialdir=initial_dir)
        if os.path.splitext(archive_file)[1] in (".stream", ".gpu_resources"):
            archive_file = os.path.splitext(archive_file)[0]
    ...
    def load_patch(
        self,
    ):  # TO-DO: only import if DIFFERENT from original audio; makes it possible to import different mods that change the same soundbank
        ...
        progress_window = ProgressWindow(
            title="Loading Files", max_progress=len(patch_file_reader.audio_sources)
        )
        progress_window.show()
        ...
        for (
            bank
        ) in patch_file_reader.wwise_banks.values():  # something is a bit wrong here
            ...
            for new_audio in bank.get_content():
                ...
                if old_audio is not None:
                    ...
                    if (
                        old_audio.get_track_info() is not None
                        and new_audio.get_track_info() is not None
                    ):
                        new_track_info = new_audio.get_track_info()
                        old_audio.get_track_info().set_data(
        ...
        for key, music_segment in patch_file_reader.music_segments.items():
            self.file_reader.music_segments[key].set_data(
                duration=music_segment.duration,
                entry_marker=music_segment.entry_marker[1],
                exit_marker=music_segment.exit_marker[1],
            )
        ...
        if os.path.exists(folder):
            ...
            for key, value in self.file_reader.text_banks.items():
                for string_id in value.string_ids:
                    if self.file_reader.string_entries[value.language][
                        string_id
                    ].modified:
    ...
    def load_wems(self, wems=None):
        ...
        for file in wems:
            ...
            # TO-DO: require more robust check
            is_from_wwise = "WwiseProjects" in file and ".cache" in file
            has_wwise_hash = "_" in file

            if is_from_wwise and has_wwise_hash:
                basename = os.path.basename(file).split("_")[0] + ".wem"
                file_id = self.get_number_prefix(basename)
            else:
...
class StringEntryWindow:
    ...
    def __init__(self, parent, update_modified):
        ...
        self.apply_button = ttk.Button(
            self.frame, text="Apply", command=self.apply_changes
        )
        self.text_box.pack()
...
class AudioSourceWindow:
    ...
    def __init__(self, parent, play, update_modified):
        ...
        self.play_button = ttk.Button(
            self.frame, text="\u23f5", image=self.fake_image, compound="c", width=2
        )
        ...
        self.play_at_label = Label(
            self.frame, text="Play At (ms)", background="white", font=("Segoe UI", 12)
        )
        self.play_at_text = Entry(
            self.frame,
            textvariable=self.play_at_text_var,
            font=("Segoe UI", 12),
            width=50,
        )

        self.duration_label = Label(
            self.frame, text="Duration (ms)", background="white", font=("Segoe UI", 12)
        )
        self.duration_text = Entry(
            self.frame,
            textvariable=self.duration_text_var,
            font=("Segoe UI", 12),
            width=50,
        )

        self.start_offset_label = Label(
            self.frame,
            text="Start Trim (ms)",
            background="white",
            font=("Segoe UI", 12),
        )
        self.start_offset_text = Entry(
            self.frame,
            textvariable=self.start_offset_text_var,
            font=("Segoe UI", 12),
            width=50,
        )

        self.end_offset_label = Label(
            self.frame, text="End Trim (ms)", background="white", font=("Segoe UI", 12)
        )
        self.end_offset_text = Entry(
            self.frame,
            textvariable=self.end_offset_text_var,
            font=("Segoe UI", 12),
            width=50,
        )
    ...
    def set_audio(self, audio):
        ...
        self.play_button.configure(
            command=partial(
                ...
                self.play_button,
                audio.get_short_id(),
                partial(reset_button_icon, self.play_button),
            )
        )
    ...
    def apply_changes(self):
        ...
        new_track_info.set_data(
            play_at=float(self.play_at_text_var.get()),
            begin_trim_offset=float(self.start_offset_text_var.get()),
            end_trim_offset=float(self.end_offset_text_var.get()),
            source_duration=float(self.duration_text_var.get()),
        )
...
class MusicSegmentWindow:
    def __init__(self, parent, update_modified):
        ...
        self.fade_in_label = Label(
            self.frame,
            text="End fade-in (ms)",
            background="white",
            font=("Segoe UI", 12),
        )
        self.fade_in_text = Entry(
            self.frame,
            textvariable=self.fade_in_text_var,
            font=("Segoe UI", 12),
            width=50,
        )

        self.fade_out_label = Label(
            self.frame,
            text="Start fade-out (ms)",
            background="white",
            font=("Segoe UI", 12),
        )
        self.fade_out_text = Entry(
            self.frame,
            textvariable=self.fade_out_text_var,
            font=("Segoe UI", 12),
            width=50,
        )
        self.revert_button = ttk.Button(self.frame, text="\u21b6", command=self.revert)
    ...
    def apply_changes(self):
        self.segment.set_data(
            duration=float(self.duration_text_var.get()),
            entry_marker=float(self.fade_in_text_var.get()),
            exit_marker=float(self.fade_out_text_var.get()),
        )
        self.update_modified()
...
class MainWindow:
    ...
    def __init__(self, config: config.Config, file_handler, sound_handler):
        ...
        self.search_bar = Entry(
            self.top_bar, textvariable=self.search_text_var, font=("Arial", 16)
        )
        ...
        self.top_bar.create_window(
            WINDOW_WIDTH - 350, 3, window=self.search_bar, anchor="nw"
        )
        self.top_bar.create_window(
            WINDOW_WIDTH - 375, 5, window=self.up_button, anchor="nw"
        )
        self.top_bar.create_window(
            WINDOW_WIDTH - 400, 5, window=self.down_button, anchor="nw"
        )
        self.top_bar.create_window(
            WINDOW_WIDTH - 100, 5, window=self.search_label, anchor="nw"
        )
        ...
        self.treeview = ttk.Treeview(
            self.root, columns=("type",), height=WINDOW_HEIGHT - 100
        )
        ...
        self.entry_info_panel = Frame(
            self.root, width=int(WINDOW_WIDTH / 3), bg="white"
        )
        self.entry_info_panel.pack(side="left", fill="both")

        self.audio_info_panel = AudioSourceWindow(
            self.entry_info_panel, self.play_audio, self.check_modified
        )
        self.event_info_panel = EventWindow(self.entry_info_panel, self.check_modified)
        self.string_info_panel = StringEntryWindow(
            self.entry_info_panel, self.check_modified
        )
        self.segment_info_panel = MusicSegmentWindow(
            self.entry_info_panel, self.check_modified
        )
        ...
        self.load_archive_menu.add_command(
            label="From File Explorer", command=self.load_archive
        )
        self.file_menu.add_cascade(menu=self.load_archive_menu, label="Load Archive")
        ...
        self.file_menu.add_command(
            label="Refresh Workspace", command=self.render_workspace
        )
        self.file_menu.add_command(
            label="Open Folder as Workspace", command=self.add_new_workspace
        )
        ...
        self.dump_menu.add_command(
            label="Dump all as .wav", command=self.dump_all_as_wav
        )
        self.dump_menu.add_command(
            label="Dump all as .wem", command=self.dump_all_as_wem
        )
    ...
    def workspace_view_on_right_click(self, event):
        ...
        if len(selects) == 1:
            ...
            is_workspace = inode.isdir \
                    and inode.absolute_path in self.config.workspace_paths
            if is_workspace:
        ...
        self.workspace_view_right_click_menu.add_command(
            label="Import", command=lambda: self.file_handler.load_wems(wems=wems)
        )
    ...
    def treeview_on_right_click(self, event):
        try:
            types = {
                self.treeview.item(i)["values"][0] for i in self.treeview.selection()
            }
            self.right_click_menu.delete(0, "end")
            self.right_click_id = self.treeview.item(self.treeview.selection()[-1])[
                "tags"
            ][0]
            is_single_select = len(self.treeview.selection()) == 1
            self.right_click_menu.add_command(
                label=("Copy File ID" if is_single_select else "Copy File IDs"),
                command=self.copy_id,
            )

            if "Audio Source" not in types:
                return
    ...
    def treeview_on_double_click(self, event):
        if (
            len(self.treeview.selection()) == 1
            and self.treeview.item(self.treeview.selection())["values"][0]
            == "Audio Source"
        ):
    ...
    def search_down(self):
        if len(self.search_results) > 0:
            ...
            self.search_label["text"] = (
                f"{self.search_result_index+1}/{len(self.search_results)}"
            )
    ...
    def show_info_window(self, event):
        ...
        if selection_type == "String":
            self.string_info_panel.set_string_entry(
                self.file_handler.get_string_by_id(
                    self.treeview.item(self.treeview.selection())["tags"][0]
                )
            )
            ...
            self.event_info_panel.set_track_info(
                self.file_handler.get_event_by_id(
                    self.treeview.item(self.treeview.selection())["tags"][0]
                )
            )
            ...
            self.segment_info_panel.set_segment_info(
                self.file_handler.get_music_segment_by_id(
                    self.treeview.item(self.treeview.selection())["tags"][0]
                )
            )
        ...
        for bank in bank_dict.values():
            ...
            for hierarchy_entry in bank.hierarchy.entries.values():
                if isinstance(hierarchy_entry, MusicSegment):
                    segment_entry = self.create_treeview_entry(
                        hierarchy_entry, bank_entry
                    )
                    ...
                    if hierarchy_entry.sources[0].plugin_id == VORBIS:
                        self.create_treeview_entry(
                            self.file_handler.get_audio_by_id(
                                hierarchy_entry.sources[0].source_id
                            ),
                            bank_entry,
                        )
        for entry in self.file_handler.file_reader.text_banks.values():
            if entry.language == language:
                ...
                for string_id in entry.string_ids:
                    self.create_treeview_entry(
                        self.file_handler.file_reader.string_entries[language][
                            string_id
                        ],
                        e,
                    )
        ...
        for bank in bank_dict.values():
            ...
            for hierarchy_entry in bank.hierarchy.entries.values():
                for source in hierarchy_entry.sources:
                    if (
                        source.plugin_id == VORBIS
                        and source.source_id not in existing_sources
                    ):
        ...
        if text != "":
            ...
            if len(self.search_results) > 0:
                self.treeview.selection_set(
                    self.search_results[self.search_result_index]
                )
                self.treeview.see(self.search_results[self.search_result_index])
    ...
    def clear_treeview_background(self, item):
        self.treeview.tag_configure(
            self.treeview.item(item)["tags"][0], background="white"
        )
    ...
    def check_modified(
        self,
    ):  # could be improved: small, but noticeable lag if there are many, many entries in the tree
        for child in self.treeview.get_children():
            self.clear_treeview_background(child)
        for audio in self.file_handler.get_audio().values():
            self.treeview.tag_configure(
                ...
                background=(
                    "lawn green"
                    if audio.modified
                    or audio.get_track_info() is not None
            ...
            if (
                ...
                for item in items:
                    ...
                    while parent != "":
                        self.treeview.tag_configure(
                            self.treeview.item(parent)["tags"][0],
        ...
        for event in self.file_handler.file_reader.music_track_events.values():
            self.treeview.tag_configure(
                event.get_id(), background="lawn green" if event.modified else "white"
            )
            if event.modified:
        ...
        try:
            for string in self.file_handler.get_strings()[language].values():
                self.treeview.tag_configure(
                    string.get_id(),
                    background="lawn green" if string.modified else "white",
                )
                if string.modified:
