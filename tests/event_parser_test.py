...
from wwise_hierarchy_154 import HircEntry, Event


class TestEventParser(unittest.TestCase):

    @staticmethod
    def _test_event_read(archive: str):
        os.environ["TEST_EVENT"] = "1"
        GameArchive.from_file(archive)

    def test_action_read(self):
        logger.critical("Running test_event_read...")
        test_all_archive_sync(self._test_event_read)

    def _test_event_read_pack(self, archive: str):
        os.environ["TEST_EVENT"] = "0"
        ...
        for bank in ctrl_group.wwise_banks.values():
            ...
            ctrl_cntrs.update({
                ...
                if hirc_entry.hierarchy_type == 0x04
            }) 

        os.environ["TEST_EVENT"] = "1"
        exp_group = GameArchive.from_file(archive)
        exp_cntrs: dict[int, Event] = {}
        for bank in exp_group.wwise_banks.values():
            ...
            exp_cntrs.update({
                hirc_entry_id: hirc_entry 
                for hirc_entry_id, hirc_entry in bank.hierarchy.entries.items()
                if isinstance(hirc_entry, Event)
            }) 
    ...
    def test_event_read_pack(self):
        logger.critical("Running test_event_read_pack...")
        test_all_archive_sync(self._test_event_read_pack)
