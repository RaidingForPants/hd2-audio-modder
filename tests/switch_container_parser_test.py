...
from wwise_hierarchy_154 import HircEntry, SwitchContainer


class TestSwitchContainerParser(unittest.TestCase):

    @staticmethod
    def _test_switch_container_read(archive: str):
        os.environ["TEST_SWITCH"] = "1"
        GameArchive.from_file(archive)

    def test_switch_container_read(self):
        logger.critical("Running test_switch_container_read...")
        test_all_archive_sync(self._test_switch_container_read)

    def _test_switch_container_read_pack(self, archive: str):
        ...
        for bank in ctrl_group.wwise_banks.values():
            ...
            ctrl_cntrs.update({
                ...
                if hirc_entry.hierarchy_type == 0x06
            }) 

        os.environ["TEST_SWITCH"] = "1"
        exp_group = GameArchive.from_file(archive)
        exp_cntrs: dict[int, SwitchContainer] = {}
        for bank in exp_group.wwise_banks.values():
            ...
            exp_cntrs.update({
                hirc_entry_id: hirc_entry 
                for hirc_entry_id, hirc_entry in bank.hierarchy.entries.items()
                if isinstance(hirc_entry, SwitchContainer)
            }) 
    ...
    def test_switch_container_read_pack(self):
        logger.critical("Running test_switch_container_read_pack...")
        test_all_archive_sync(self._test_switch_container_read_pack)
